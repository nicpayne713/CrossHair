...
def make_registrations():
    ...
    _min_tz_offset, _max_tz_offset = -datetime.timedelta(hours=24), datetime.timedelta(hours=24)
    ...
    def make_date(p: Any) -> datetime.date:
        ...
        try:
            p.space.defer_assumption('Invalid date', lambda: (
    ...
    def make_time(p: Any) -> datetime.time:
        ...
        try:
            p.space.defer_assumption('Invalid datetime', lambda: (
                not _raises_value_error(orig_check_time_fields, (hour, minute, sec, usec, 0))))
    ...
    def make_datetime(p: Any) -> datetime.datetime:
        year, month, day, hour, minute, sec, usec = p(int), p(int), p(int), p(int), p(int), p(int), p(int)
        tzinfo = p(datetime.tzinfo) if p.space.smt_fork() else None
        try:
            p.space.defer_assumption('Invalid datetime', lambda: (
                not _raises_value_error(orig_check_date_fields, (year, month, day)) and
                not _raises_value_error(orig_check_time_fields, (hour, minute, sec, usec, 0))
            ))
